...
function calcularBiseccion() {
    ...
    while (
        ...
        try {
            ...
            mensaje.innerHTML = "No se pudo evaluar la función.";
            return;
        }

        if (!Number.isFinite(fxm)) {
            mensaje.innerHTML = "La función no está definida en uno de los puntos evaluados.";
            return;
        }
        ...
        fila.innerHTML = ` <td>${iteracion}</td> <td>${formatearNumero(a)}</td>
            <td>${formatearNumero(b)}</td> <td>${formatearNumero(xm)}</td>
            <td>${formatearNumero(fxm)}</td> <td>${formatearNumero(error)}</td> `;
    ...
    // MOSTRAR RESULTADO
    mostrarResultado(raiz, iteracion, a, b, tolerancia);
